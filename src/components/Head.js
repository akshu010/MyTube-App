...
const Head = () => {
  ...
  const getSearchSuggestions = async () => {
    ...
    // Assuming the API returns video ID along with titles
    const titlesWithIds = json?.items.map((item) => ({
      title: item?.snippet?.title,
      id: item?.id?.videoId, // Extract the video ID
    }));

    setSuggestions(titlesWithIds);
    dispatch(cacheResults({ [searchQuery]: titlesWithIds }));
  };
  ...
  const handleSuggestionClick = (suggestion) => {
    // Navigate to the watch page using the video ID
    navigate(`/watch?v=${suggestion.id}`); // Make sure suggestion.id contains the video ID
  };

  return (
    <div className="flex justify-between px-6 py-6 shadow-xl items-center text-center sticky top-0 bg-white z-50">
      <div className="flex gap-6">
        <i
          onClick={() => handleToggleMenu()}
          className="text-2xl cursor-pointer ri-menu-line"
      ...
      <div className="relative flex-1">
        <div>
          <input
            className="h-10 w-full border border-black p-2 px-6 rounded-2xl"
            type="text"
            placeholder="Search Here"
            value={searchQuery}
            onChange={(e) => setSearchQuery(e.target.value)}
        ...
        {showSuggestions && suggestions.length > 0 && (
          <div className="absolute bg-white w-[45vw] text-start px-5 py-2 rounded-lg shadow-lg mt-1">
            <ul>
              {suggestions.map((s, index) => (
                <li
                  key={index}
                  className="py-3 shadow-sm cursor-pointer hover:bg-gray-100 rounded-lg"
                  onClick={() => handleSuggestionClick(s)} // Click handler
                >
                  <i className="ri-search-line"></i> {s.title}
                </li>
              ))}
            </ul>
          </div>
